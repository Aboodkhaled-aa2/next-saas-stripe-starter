...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-[#020617] text-white p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      ...
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm text-slate-400 opacity-70 ring-offset-slate-950 transition-opacity hover:text-white hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:pointer-events-none">
        <X className="size-5" />
